     ...
     char* argv[])
{
    ...
    if(argc<0) {
        ...
        if(options[FILELIST].doesOccur && argc!=1) {
            fprintf(stderr, "%s: cannot combine --fileList with a command line file list\n", argv[0]);
            illegalArg = TRUE;
    ...
    if(illegalArg || options[HELP1].doesOccur || options[HELP2].doesOccur) {
        ...
        fprintf(stderr,
                "\t      --fileList           Read in the list of resource bundle source files from a file.\n"
    ...
    // List of source input files.
    std::vector<std::string> list;

    // The list of input files can either come from a separate file, or directly on the command line itself.
    if(fileList) {
        std::ifstream f(fileList);
        if (f.fail()) {
            std::cerr << argv[0] << " error: unable to open the file: " << fileList << std::endl;
            return U_FILE_ACCESS_ERROR;
        }
        std::string currentLine;
        while (std::getline(f, currentLine)) {
            // Ignore # comments and empty lines in the input file.
